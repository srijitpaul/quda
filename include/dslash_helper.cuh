...
namespace
{
  __device__ cuda::atomic<long, cuda::thread_scope_system> intersync {0};     // = ATOMIC_VAR_INIT(0);
  __device__ cuda::atomic<long, cuda::thread_scope_device> interior_done {0}; // = ATOMIC_VAR_INIT(0);
  __device__ cuda::atomic<int, cuda::thread_scope_block> interior_count {0};  // = ATOMIC_VAR_INIT(0);
} // namespace
#endif
  ...
  */
  ...
  __global__ void dslashGPU(Arg arg)
  {
    ...
    if (kernel_type == INTERIOR_KERNEL && blockIdx.x < arg.pack_blocks) {
      ...
      // for uber kernel do not specialize on parity
      if (true) {
        // if (!(arg.shmem & 64)) {
